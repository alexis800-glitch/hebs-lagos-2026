...
const GLOBAL_STAGES = [
  {
    ...
    desc: 'Submit a 3-minute MP4/MOV video to casting@hebslagos.com. Show your name, category, work-in-progress clips, and a before/after transformation. Pay the $50 entry fee at hebslagos.com to complete registration.',
  },
  {
    n: '02',
...
function PartnerBanner({ logo, name, role, note }: { logo: string; name: string; role: string; note: string }) {
  return (
    <div className="flex items-center gap-4 border border-white/[0.06] bg-zinc-900/30 rounded-xl px-5 py-4">
      ...
      <p className="text-zinc-500 text-xs leading-relaxed ml-auto hidden md:block max-w-xs text-right shrink-0">
        {note}
      </p>
    </div>
  )
...
function StatCell({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="text-center">
      ...
      <p className={`text-xs font-semibold ${accent ? 'text-amber-500/90' : 'text-white'}`}>{value}</p>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        {div.prizes.map((p) => (
          <p key={p} className="text-zinc-400 text-xs font-mono">{p}</p>
        ))}
      </div>
      ...
      <main className="w-full min-h-screen bg-zinc-950 text-white">
        ...
        <div className="pt-32 pb-10 px-5 sm:px-8 text-center">
          ...
          <p className="text-zinc-400 text-sm sm:text-base mt-4 max-w-lg mx-auto leading-relaxed">
            Three championship tracks. $35,000+ in prizes. One stage. Lagos, Nigeria — October 2026.
          </p>
        </div>
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
                  ...
                  <div className="flex-shrink-0 md:text-right">
                    ...
                    <div className="flex flex-col gap-2">
                      {[
                        { place: '1st', amount: '$20,000', color: 'text-amber-400' },
                        { place: '2nd', amount: '$10,000', color: 'text-zinc-300' },
                        { place: '3rd', amount: '$5,000',  color: 'text-zinc-500' },
                      ].map(({ place, amount, color }) => (
                        <div key={place} className="flex items-center md:justify-end gap-3">
                          <span className={`font-mono text-xs ${color}`}>{place}</span>
                          <span className="text-white font-semibold text-sm tabular-nums">{amount}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex flex-col gap-12">
              ...
              <div className="flex flex-col items-center gap-3 pt-2">
                <p className="text-zinc-500 text-xs font-mono tracking-widest uppercase text-center">
                  Registration opens closer to 2027
                </p>
                <PrimaryCTA href="https://hebseventportal.com">
                  Stay Updated
                </PrimaryCTA>
                <p className="text-zinc-600 text-xs font-mono">
                  Sign up at hebseventportal.com to be notified when registration opens
                </p>
              </div>
            </div>
          )}
